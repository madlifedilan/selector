...
class Seizer(object):
    ...
    def __init__(self, index, target_host, cookies, info, timeout):
        ...
        self.whole_url = target_host + "/stu1/"  #选课地址
        ...
        if not self.cookies:
            logging.warning("{}:请先登录教务管理系统".format(self.process_name))
            return
    ...
    def load_whole_page(self):
        ...
        try:
            ...
            if re.findall(r'name="csrfmiddlewaretoken" value="(.*?)"', res.text):
                res = self.submit_whole(res, self.info.courseId)
                return res
            return None
    ...
    def get_seizer_tokens(self):
        """
        改变全校请选课的显示数量
        :param res: 全校性选课的页面html
        :return:
        """
        login_page_url = "http://127.0.0.1:8000/stu1/"

        res_token = self.session.get(login_page_url, headers=self.session.headers)
        self.token = etree.HTML(res_token.text).xpath('//*[@name="csrfmiddlewaretoken"]/@value')
        # print(self.token)
        return

    @Until.until
    def submit_whole(self, res, id):
        """
        提交全校选课表单
        :return:
        """
